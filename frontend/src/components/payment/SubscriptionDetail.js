...
const SubscriptionDetail = () => {
  ...
  // Grab the plan from state passed from PricingPage (make stateful so /verify can populate)
  const [selectedPlan, setSelectedPlan] = useState(
    location.state?.plan || { name: "Growth Plan", price: "990", currency: "NPR" },
  );
  ...
  // If a `data` query param is present (e.g. after redirect from eSewa or verification link), call backend to verify and prefill
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/subscription/verify?data=${encodeURIComponent(payload)}`);
        if (!res.ok) {
        ...
        if (json.success && json.subscription) {
          ...
          // prefill form and plan from server-verified payload
          setFormData((f) => ({ ...f, orgName: s.orgName || f.orgName, billingEmail: s.billingEmail || f.billingEmail }));
          if (s.planName || s.amount) {
            setSelectedPlan({ name: s.planName || selectedPlan.name, price: String(s.amount || selectedPlan.price), currency: 'NPR' });
          }
        } else {
          console.warn('Verify returned no subscription', json);
        }
      } catch (err) {
